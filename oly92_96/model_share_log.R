...
holy$gdp_96 <- log(holy$gdp_96)
holy$X1996 <- log(holy$X1996)
...
(rf <- randomForest(Share_96 ~ Share_92 + log(gdp_96) + log(X1996),
                    data = oly3, ntree = 5000, mtry = 3,
                    na.action = na.omit))
randomf_rmse <- sqrt(mean(rf$mse))

(ds_rf <- randomForest(Share_96 ~ Share_92 + log(gdp_96) + log(X1996),
                       data = ds, ntree = 5000, mtry = 3,
                       proximity = TRUE))
